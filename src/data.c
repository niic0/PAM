...
void init_data(DATA* sample, size_t nbr_objets, size_t size_objets) {
  ...
  if(sample->objets == NULL) {
    perror("Erreur d'allocation dans la fonction init_data()");
    exit(EXIT_FAILURE);
  }
}
 ...
 * - data data_set : Dataset contenant
 */
DATA set_data_csv(char* file_name) {
  ...
  char line[1024];
  char* tok;
  size_t size_objet = size_objets(file_name);
  size_t nbr_objet = nbr_objets(file_name);

  init_data(&dataset, nbr_objet, size_objet);
  ...
  for (int i=-1; fgets(line, 1024, stream); i++) {
    ...
    dataset.objets[i].point = malloc(sizeof(int)*size_objet);
 ...
 * - size_t nbr_objets  : Nombre d'objet voulue dans le dataset
 * - size_t size_objets : Nombre de point qui définissent un objet
 * - size_t range       : Valeurs maximal des valeurs aléatoires
*/
...
*/
size_t size_objets(char* file_name) {
  FILE* stream = fopen(file_name, "r");
  size_t size_objets = 0;
  char line[1024];
  char* tok;

  fgets(line, 1024, stream); // ligne 1 du fichier
 ...
 * - char* file_name : Nom du fichier csv
*/
size_t nbr_objets(char* file_name) {
  FILE* stream = fopen(file_name, "r");
  size_t nbr_objets = 0;
  char line[1024];
  ...
  // On decremente de 1 car la premiere ligne correspond aux caracteristiques des valeurs
  return nbr_objets-1;
}
...
/*
 * Ajoute un objet à un dataset. Retourne 1 si la fonction a planté, 0 sinon.
...
int add_objet(DATA* sample, OBJET obj_to_add) {
  ...
  if (sample->objets == NULL) {
    fprintf(stderr, "Erreur a l'allocation de la memoire (fct add_objet)\n");
    return 1;
  }
...
/*
 * Cree k clusters selon les rang des medoid dans le dataset
